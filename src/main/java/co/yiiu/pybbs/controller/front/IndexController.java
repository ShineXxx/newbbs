...
//        if (subject.isAuthenticated()) return redirect("/admin/index");
        if (subject.isAuthenticated()) return redirect("/admin/topic/list");
        return "admin/login";
    }
...
//                .getRequestUrl();
        String url = WebUtils.getSavedRequest(request) == null ? "/admin/topic/list" : WebUtils.getSavedRequest(request)
                .getRequestUrl();
        return redirect(url);
    }
